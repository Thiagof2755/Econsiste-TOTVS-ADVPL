...
Else
	...
	lSemCbenef 		  := date() >= CTOD("02/09/2019") .or. cAmbiente == "2"
Endif
...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de operação da nf sobre cupom
                  ...
                  If MsSeek(xFilial("SFT")+"S"+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_NFORI)
                    ...
                    Else
                      ...
                      Endif
                    EndIf
                  Else//Cuztomização Wise 10/10/2023 - Para Buscar a chave da Nf Origem mesmo com cliente diferente
                    dbSelectArea("SFT")
                    dbSetOrder(1)
                    If MsSeek(xFilial("SFT")+"S"+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_NFORI)
                      If Alltrim(SFT->FT_CFOP) $ cMVREFNFE
                        //Tratamento para que leve na TAG <refNFe> as notas referenciadas que contém o CFOP no parâmetro MV_REFNFE
                        dbSelectArea("SF3")
                        dbSetOrder(6)
                        If (MsSeek(xFilial("SF3")+SFT->FT_NFISCAL+SFT->FT_SERIE))
                          _cCGC := Posicione("SA1",1,xFilial("SA1")+SF3->(F3_CLIEFOR + F3_LOJA),"A1_CGC")
                          If cChave <> dToS( SF3->F3_EMISSAO ) + SF3->F3_SERIE + SF3->F3_NFISCAL + _cCGC + SM0->M0_ESTCOB + SF3->F3_ESPECIE + SF3->F3_CHVNFE;
                            .or. ( cAliasSD2 )->D2_ITEM <> cItemOr
                          
                            aAdd( aNfVinc, { SF3->F3_EMISSAO, SF3->F3_SERIE, SF3->F3_NFISCAL, _cCGC, SM0->M0_ESTCOB, SF3->F3_ESPECIE, SF3->F3_CHVNFE } )
                            cChave	:= dToS( SF3->F3_EMISSAO ) + SF3->F3_SERIE + SF3->F3_NFISCAL + _cCGC + SM0->M0_ESTCOB + SF3->F3_ESPECIE + SF3->F3_CHVNFE											
                            lVinc := .T.
                          endIf
                          cItemOr	:= ( cAliasSD2 )->D2_ITEM
                        Endif
                    ...
                    //Fim da Customização Wise 10/10/2023
                  EndIf
                EndIf
              Else
                  aOldReg  := SD2->(GetArea())
      ...
      //						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
            ...
            lAmarra := fAmarracao(SC5->C5_TIPO,(cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD))

						If (!Empty((cAliasSD2)->D2_IDENTB6) .And. lNFPTER  ) .or. lAmarra
              If SC5->C5_TIPO == "N" 
...
IF lPe01Nfe     
	//Rudson/eConsiste - 18/08/22 - Inclusão de arrays de impostos quando aglutina produto, aglutinar os impostos tambem
	...
	aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aEspVol,aNfVinc,aDetPag,aObsCont,;
						aICMS,aIcmsSt,aIcmsZFM,aIpi,aPis,aCofins,aPisST,aISSQN,aCST,aCofinsST,aPedCom}
...
cString += '<vUnCom>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[09],21,5))+'</vUnCom>'
...
Endif
cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[12],21,5))+'</vUnTrib>'	
...
// ** Alterado ** Cassiano Mendes - Econsiste 30/01/2020 
cVunCom := ConvType(aProd[10]/aProd[09],21,5)
cVunTrib:= ConvType(aProd[10]/aProd[12],21,5)
//**********************************************
...
/*/
//-----------------------------------------------------------------------
static function getCodLan( cUF, cCST )
...
return cCodlan
//---------------------------------------------------------------------
Static Function fAmarracao(cTipo,cChave)
  local lRet := .f.

	If cTipo == "N" 
    //--A7_FILIAL + A7_CLIENTE + A7_LOJA + A7_PRODUTO
    SA7->(dbSetOrder(1)) 	         
    If SA7->(MsSeek( xFilial("SA7") + cChave )) .and. !empty(SA7->A7_CODCLI) .and. !empty(SA7->A7_DESCCLI) 
      lRet := .t.
    EndIf 
  ElseIf cTipo == "B"
  //--A5_FILIAL + A5_FORNECE + A5_LOJA + A5_PRODUTO
    SA5->(dbSetOrder(1)) 	         
    If SA5->(MsSeek( xFilial("SA5") + cChave )) .and. !empty(SA5->A5_CODPRF) .and. !empty(SA5->A5_DESREF)
      lRet := .t.    
    EndIf 	
  EndIf  
return lRet
