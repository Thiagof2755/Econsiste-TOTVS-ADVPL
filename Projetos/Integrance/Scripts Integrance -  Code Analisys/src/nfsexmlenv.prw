...
user function nfseXMLEnv( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
	...
	Private lMvded		:= SuperGetMV("MV_NFSEDED",,.F.)
			...
			If !Empty(SF2->F2_DUPL)	
				...
				While !Eof() .And. xFilial("SE1") == (cAliasSE1)->E1_FILIAL .And.;
					...
					If 	(cAliasSE1)->E1_TIPO = MVNOTAFIS .OR. ((cAliasSE1)->E1_ORIGEM = 'LOJA701' .AND. (cAliasSE1)->E1_TIPO $ cWhere)
						...
						aadd(aDupl,{(cAliasSE1)->E1_PREFIXO+(cAliasSE1)->E1_NUM+(cAliasSE1)->E1_PARCELA,(cAliasSE1)->E1_VENCORI,(cAliasSE1)->E1_VALOR,(cAliasSE1)->E1_PARCELA,(cAliasSE1)->E1_NUM})
           ...
           If cMvMsgTrib $ "1-3"
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				Else
					...
					ElseIf Alltrim(SM0->M0_CODMUN) == "3507605" .And. SF4->F4_ISSST == '3'			// Bragança Paulista
				...
				If !lNFeDesc
					...
					ElseIf lNatOper .And. lSC5 .And. !Empty(SC5->C5_MENNOTA).And. nCont == 1
						...
						cNatOper += "$$$"
					ElseIf !AllTrim(SF2->F2_MENNOTA) $ cMensCli .and. !Empty(AllTrim(SF2->F2_MENNOTA))
				...
				dbSelectArea("SX5")
				dbSetOrder(1)
				If dbSeek(xFilial("SX5")+"60"+(cAliasSD2)->D2_CODISS)
					If !lNFeDesc
						If nCont == 1 
							cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(FWGetsx5("60", (cAliasSD2)->D2_CODISS),1,55))),AllTrim(SubStr(FWGetsx5("60", (cAliasSD2)->D2_CODISS),1,55)))    			
	    				EndIf
		    		ElseIf nCont == 1 
							cDescrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(FWGetsx5("60", (cAliasSD2)->D2_CODISS),1,55))),AllTrim(SubStr(FWGetsx5("60", (cAliasSD2)->D2_CODISS),1,55)))    			
		    		EndIf
    			EndIf
					...
					If Len(aProd) > 0			
						...
						If nX > 0						
							...
							If SM0->M0_CODMUN == "3550308"					
								...
								aProd[nx][28]+=	A410Arred( IIF(!(cAliasSD2)->D2_TIPO $ "IP",(cAliasSD2)->D2_TOTAL,0) + ((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR), "D2_TOTAL")	//Valor Total				
							Else
				...
				If !lAglutina .Or. Len(aProd) == 0
					...
					aadd(aProd,	{Len(aProd)+1,;
								...
								SB1->B1_UM,;
								(cAliasSD2)->D2_QUANT,;
								A410Arred(FunValUnit((cAliasSD2)->D2_TIPO, (cAliasSD2)->D2_PRCVEN, (cAliasSD2)->D2_QUANT,(cAliasSD2)->D2_VALICM), "D2_TOTAL"),;
								...
								IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_PRCVEN+IIf(SM0->M0_CODMUN == "4205407",0,(((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR)/(cAliasSD2)->D2_QUANT)),0),;
								...
								(cAliasSD2)->D2_VALFRE,;
								A410Arred( IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(SM0->M0_CODMUN == "3550308",(cAliasSD2)->D2_PRCVEN * (cAliasSD2)->D2_QUANT,(cAliasSD2)->D2_TOTAL),0), "D2_TOTAL"),; // Valor Liquido
								A410Arred(FunValTot((cAliasSD2)->D2_TIPO,(cAliasSD2)->D2_PRCVEN, (cAliasSD2)->D2_QUANT, (cAliasSD2)->D2_TOTAL, (cAliasSD2)->D2_DESCON, (cAliasSD2)->D2_DESCZFR, (cAliasSD2)->D2_VALICM), "D2_TOTAL"),; //Valor Total
								SF3->F3_ISSSUB + SF3->F3_ISSMAT + Iif((cAliasSD2)->D2_DESCON > 0 .and. lMvded,(cAliasSD2)->D2_DESCON,0),; //Valor Total de deducoes.
...
static function prestacao( cMunPrest, cDescMunP, aDest, cMunPSIAFI )
	...
	if empty( cMunPSIAFI )
		...
		If  CC2->(DbSeek(xFilial("CC2")+ allTrim(SC5->C5_ESTPRES)+ cIbge)) 
	...
	cString	+= "<logradouro>" + IIf(!Empty(Alltrim(cLogradPres)),Alltrim(cLogradPres),IIf(!Empty(aDest[3]),allTrim(aDest[3]),"")) + "</logradouro>"
	cString	+= "<numend>" + IIf(!Empty(Alltrim(cNumEndPres)),Alltrim(cNumEndPres), allTrim(aDest[4])) + "</numend>"
	...
	cString	+= "<bairro>" + IIf(!Empty(Alltrim(cBairroPres)),Alltrim(cBairroPres),IIf(!Empty(aDest[6]),allTrim( aDest[6] ),"")) + "</bairro>"
	...
	cString	+= "<codpais>" + IIf(!Empty(Alltrim(cPaisPres)),Alltrim(cPaisPres), IIf(!Empty(aDest[11]),allTrim(aDest[11]),"")) + "</codpais>"
	cString	+= "<cep>" + IIf(!Empty(Alltrim(cCepPres)),Alltrim(cCepPres),IIf(!Empty(aDest[10]),allTrim( aDest[10] ),"" )) + "</cep>"

	cString	+= "<tipologr>" + IIf(!Empty(Alltrim(cLogradPres)),RetTipoLogr(Alltrim(cLogradPres)),IIf(!Empty(aDest[3]),RetTipoLogr(allTrim(aDest[3])),"2")) + "</tipologr>"
	cString	+= "</prestacao>"
...
Static Function tomador( aDest )
	...
	If !lSemTomador 		
		...
		If !Empty(allTrim( aDest[26] ))
			cString	+= "<doctomestra>"+ allTrim( aDest[26] ) +"</doctomestra>"
		Endif
		If SM0->M0_CODMUN == '3550308'			
		...
		EndIf
		If Len( cCodTom ) <= 5 .And. (!(cCodTom $ '99999').Or. (cCodTom $ '99999' .And. SM0->M0_CODMUN $ '3550308|3552205|4205407|3300704|3156700|4115200'))
	...
	if SA1->(FieldPos("A1_TPESSOA")) > 0 .And. !Empty(SA1->A1_TPESSOA).and. getVersaoTSS() >='2.57'
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon   )	
	...
	for nX := 1 to len( aProd )
		...
		If ( SC6->(FieldPos("C6_DESCRI")) > 0 .And. Len(aProd[nX]) > 40 .And. !Empty(aProd[nX][41]) ) .And. (!lNFeDesc .And. !GetNewPar("MV_NFESERV","1") == "1" .And. !Empty(zGetmv("MV_CMPUSR")) )
		...
		cString	+= "<valir>" + allTrim( convType(Iif(Len(aIRRFXml[3]) > 1 .And. Len(aProd ) > 1 .And. cCodMun $ "2610707-4305108",(aIRRFXml[3][Nx]+nRatVIRRF),aIRRFXml[1]),15,2 )) + "</valir>"
...
static function pagtos( aDupl )
	...
	If len( aDupl ) > 0
		cString	+= "<pagamentos>"
		cString += "<condpag>"+Iif(Len(aDupl) > 1, "3", "1")+ "</condpag>"
