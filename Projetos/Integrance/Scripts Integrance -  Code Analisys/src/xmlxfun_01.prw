...
Default lValid := .T.
Default lGatilho := .T.
FWLogMsg("INFO",,,,,,"fXMLcpo: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Iniciando...")
...
FWLogMsg("INFO",,,,,,"fXMLcpo: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Concluido!")
Return lRetVal
...
Local oXml := nil
Default cXml := ""
FWLogMsg("INFO",,,,,,"c2oXML: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Iniciando...")
// TRY EXCEPTION
oXml := XmlParser(cXml, cDelimit, @cError, @cWarning)
// CATCH EXCEPTION
// END TRY
If ValType(oXml) != "O" // Nao teve exito na geracao do Objeto
	FWLogMsg("ERROR",,,,,,"c2oXML: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Falha na geracao do objeto a partir do XML!")
Else // Sucesso
	FWLogMsg("INFO",,,,,,"c2oXML: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Sucesso na geracao do objeto a partir do XML!")
EndIf
FWLogMsg("INFO",,,,,,"c2oXML: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Conteudo de cXml:")
FWLogMsg("INFO",,,,,,cXml)
FWLogMsg("INFO",,,,,,"c2oXML: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Concluido!")
Return oXml
...
Private aItens := {}
Private nI
FWLogMsg("INFO",,,,,,"TrataSB1: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Iniciando...")
...
EndIf
FWLogMsg("INFO",,,,,,"TrataSB1: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Concluido!")
...
Local lInclPRD := .F.
Default xAliqIPI := 0
FWLogMsg("INFO",,,,,,"BuscaSB1: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Iniciando...")
/*
if _tipoNF != "D"
SB1->( dbSetOrder(1) )
...
FWLogMsg("INFO",,,,,,"BuscaSB1: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Concluido!")
RestArea(aArea)
Return nRecord
...
Local nTabIcms := 0
FWLogMsg("INFO",,,,,,"CriarSB1: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Iniciando...")
...
//Alteracao 24/06/2024 T๚lio
cEmpresa := FWGrpEmp()
cAliasTmp := GetNextAlias()
cFiltro   := ""
OpenSXs(NIL, NIL, NIL, NIL, cEmpresa, cAliasTmp, "SX5", NIL, .F.)
(cAliasTmp)->(DbSetOrder(1)) // X5_FILIAL+X5_TABELA+X5_CHAVE
...
*/
//	restArea( aArea )
FWLogMsg("INFO",,,,,,"CriarSB1: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Concluido!")
...
Local cFiltro   := ""
SAH->(DbSetOrder())
// Abrir a SX5 com um alias temporแrio
OpenSXs(NIL, NIL, NIL, NIL, cEmpresa, cAliasTmp, "SX5", NIL, .F.)
(cAliasTmp)->(DbSetOrder(1)) // X5_FILIAL+X5_TABELA+X5_CHAVE
if SAH->(!DbSeek(xFilial("SAH") + cUMorig))
    if (cAliasTmp)->( dbSeek( xFilial("SX5")+"um"+cUMorig, .T.  ) )
...
Default xDescri  	:= ""
Default xProd    	:= ""
FWLogMsg("INFO",,,,,,"TrataImp: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Iniciando...")
//	if _tipoNF == "D"
If _tipoNF $ "D;CTE"
	// I C M S
...
If _tipoNF != "D" // GINFES e Prefeitura
	...
	EndIf
EndIf
FWLogMsg("INFO",,,,,,"TrataImp: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Concluido!")
RestArea(aArea)
Return aClone(aRet)
...
Local xVal		:= Nil
Local lIpiTrib	:= .F.
FWLogMsg("INFO",,,,,,"ConsTes: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Iniciando...")
...
If Select(cTmpAlias) > 0
	(cTmpAlias)->(DbCloseArea())
EndIf
FWLogMsg("INFO",,,,,,"ConsTes: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Concluido!")
Return
...
If Aviso("Importa็ใo XML em Lote", "Somente DANFE ou todos os arquivos?", {"DANFE", "Todos"}) == 1
	...
	Else
		...
		if lContinua
			...
			elseif MV_PAR01 == 3
				XMLTRB->( dbSetOrder(4) )	// Destinatแrio
			else
			...
			while ! XMLTRB->( EOF() )
				...
				if isInCallStack("MATA410") .or. isInCallStack("MATA920")
					...
					cMsg += " - Emissใo: "+ DtoC(XMLTRB->EMISSAO) + CRLF
