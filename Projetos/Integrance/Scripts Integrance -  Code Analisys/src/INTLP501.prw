...
±±ºPrograma  ³ INTLP501 ºAutor ³ Jonathan Schmidt Alvesº Data ³22/05/2019 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Customizacao para tratamento do LP 501 (Inclusao de Adtos  º±±
±±º          ³ a Receber) Pode ser usado tambem no 502 (oposto).          º±±
±±º          ³ LP: 501: Contas a Receb - Inc Titulos Recb Antecipado (RA) º±±
±±º          ³ LP: 502: Contas a Receb - Exc Titulos Recb Antecipado (RA) º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±º          ³ Parametros:                                                º±±
...
User Function INTLP501(cPrc, cSeq, cHis, lRat)
Local xRet := Nil
Local aArea := GetArea()
...
ConOut("INTLP501: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Iniciando...")
ConOut("INTLP501: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Prc: " + cPrc + " Seq: " + cSeq)
ConOut("INTLP501: " + DtoC(Date()) + " " + Time() + " " + cUserName + " E1_PREFIXO/E1_NUM/E1_PARCELA/E1_TIPO: " + SE1->E1_PREFIXO + "/" + SE1->E1_NUM + "/" + SE1->E1_PARCELA + "/" + SE1->E1_TIPO)
If cPrc == "VLR" // Valor
...
RestArea(aAreaSF1)
RestArea(aArea)
ConOut("INTLP501: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Retorno: " + Iif(cPrc == "VLR", cValToChar(xRet), xRet))
ConOut("INTLP501: " + DtoC(Date()) + " " + Time() + " " + cUserName + " Concluido!")
Return xRet