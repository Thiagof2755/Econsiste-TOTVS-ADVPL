...
±±³Descri‡…o ³Exemplo de geracao da Nota Fiscal Digital de Serviços, para ³±±
	...
	if lCmpFin
		...
		If SE2->( DbSeek(xFilial("SE2")+cClieFor+cLoja+cSerie+cNota) .And. !Empty( SE2->E2_CODISS ) )
			...
			aadd(aProd,	{Len(aProd)+1,;
								...
								0,; //30
								0,; //31
								FIM->FIM_CDTRIB,;//"2301",; //32 - Codigo de Tributação Municipio - B1_TRIBMUN
	...
	If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
		...
		While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			Else	
				...
				If Empty(cNatOper)
					cNatOper := AllTrim(SubStr(FWGetsx5("13", SF4->F4_CF),1,55))
    			EndIf
    		EndIf 
			...
			If dbSeek(xFilial("SX5")+"60"+RetFldProd(SB1->B1_COD,"B1_CODISS"))
				cDescServ := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(FWGetsx5("60", RetFldProd(SB1->B1_COD,"B1_CODISS")),1,55))),AllTrim(SubStr(FWGetsx5("60", RetFldProd(SB1->B1_COD,"B1_CODISS")),1,55)))    			
   			EndIf
			...
			Else
				...
				If Alltrim(SM0->M0_CODMUN) == "3507605" .And. SF4->F4_ISSST == '3'			// Bragança Paulista
...
Else
	...
	If DbSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)	
		...
		*/
		...
		//³Expressão para incluir campos na pesquisa de itens da nota ³
...
ElseIf Alltrim(SM0->M0_CODMUN) $ "3550308-3304557-3303906-2927408-3525300" .And. aNota[04] == "0" .or. aNota[04] == "3" // NFTS: São Paulo, Rio de Janeiro, Petrópolis e Salvador
...
if aNota[4] == "0"
	cString += '<TipoDoDocumento>02</TipoDoDocumento>'
else
 	if aScan(aMVTitNFT,{|x| x[1] == aNota[5]}) == 1
   		cString += '<TipoDoDocumento>01</TipoDoDocumento>'
   	else
		cString += '<TipoDoDocumento>03</TipoDoDocumento>'   	
   	endif 	
endif	
...
If !Alltrim(SM0->M0_CODMUN) $ "3305505-3152501-2927408"       
	Do Case
		Case aTotal[3] $ "2"
	...
	EndCase
ElseIf Alltrim(SM0->M0_CODMUN) $ "2927408" 
	//De/para na tributação para NFTS de Salvador
	Do Case
		Case aTotal[3] $ "4"
			cString += '<Tributacao>I</Tributacao>' //Imune
	    Case aTotal[3] $ "5"
			cString += '<Tributacao>J</Tributacao>' //ISS suspenso por decisão judicial
		OtherWise
			cString += '<Tributacao>T</Tributacao>' //Operação normal
	EndCase
Else
	cTributa := aTotal[3]
...
If Alltrim(SM0->M0_CODMUN) $ "3168705-3305505-3550308-3304557-2927408" // Timoteo-MG # Saquarema-RJ # Sao Paulo-SP # Rio de Janeiro-RJ	
	if SM0->M0_CODMUN $ "3550308-3304557" .And. len(aPrest)> 21
		...
		//Para NFTS deve pegar a informação do Fornecedor(Prestador do Serviço)
	...
	elseif SM0->M0_CODMUN $ "2927408" .And. len(aPrest)>21
		//Para NFTS de Salvador deve pegar a informação do Fornecedor(Prestador do Serviço)
...
cString += '<MotCancelamento>'+cMotCanc+'</MotCancelamento>'    
//Inserida a mesma informação da tag <DescricaoRPS>, pois alguns municipios utiilizam a TAG Observacao, para levar a informacao.
...
For Nx := 1 to Len(aProd)
	...
	//If cCodMun $ "3303906-3525300" // Petrópolis-RJ / Jahu-SP
		//cXml += '<ItemListaServico>'+ RetCodISS(AllTrim(aProd[Nx][23])) +'</ItemListaServico>'
...
//Informações do Endereço da Prestação do Serviço
...
Função que monta as informações de onde será prestado o serviço

@author Eduardo Silva             
@since 27/04/2012
@version 1.0
                                   
@param	aDest		Dados do destinatário


@return	aEndPres	Array com as informações de onde será prestado o serviço
/*/
//-------------------------------------------------------------------
...
If Len(aMvEndPres) > 0 .And. !Empty(aMvEndPres[1])
	// Exemplo de preenchimento do parâmetro MV_ENDPRES {'C5_ENDPRES','C5_NUMPRES','C5_COMPPRE','C5_BAIPRES','C5_CEPPRES'}
	
	aAdd(aEndPres, Alltrim(IIF(!Empty(FisGetEnd(aMvEndPres[01])[1]) .and. SC5->(FieldPos(FisGetEnd(aMvEndPres[01])[1])) > 0 , SC5->&(FisGetEnd(aMvEndPres[01])[1]),aDest[3]))) //Logradouro da prestação
	aAdd(aEndPres, Alltrim(IIf(!Empty(aMvEndPres[02]) .and. SC5->(FieldPos(aMvEndPres[02])) > 0, SC5->&(aMvEndPres[02]), aDest[4] ))) //Número do logradouro da prestação
	aAdd(aEndPres, Alltrim(IIF(!Empty(aMvEndPres[03]) .and. SC5->(FieldPos(aMvEndPres[03])) > 0, SC5->&(aMvEndPres[03]), aDest[5] ))) //Complemento do logradouro da prestação
	aAdd(aEndPres, Alltrim(IIF(!Empty(aMvEndPres[04]) .and. SC5->(FieldPos(aMvEndPres[04])) > 0, SC5->&(aMvEndPres[04]), aDest[6] ))) //Bairro do logradouro da prestação
	
	If SC5->(FieldPos("C5_DESCMUN")) > 0
		aAdd(aEndPres,Alltrim(IIF ( !Empty(SC5->C5_DESCMUN), SC5->C5_DESCMUN, aDest[8] ))) //Cidade da prestação
	Else
		aAdd(aEndPres,Alltrim(aDest[8]) )
	EndIf
	
	aAdd(aEndPres, Alltrim(IIF(!Empty(aMvEndPres[05]) .and. SC5->(FieldPos(aMvEndPres[05])) > 0,  SC5->&(aMvEndPres[05]), aDest[10] ))) //Cep da prestação
	
	If SC5->(FieldPos("C5_ESTPRES")) > 0 // UF da prestação
...
Função para buscar o titulo de iss retido e retornar a data
...
While !SE2->(Eof()) .And. SE2->(E2_FILIAL+E2_PREFIXO+E2_NUM) == xFilial("SE2")+PadR(cSerie,TamSx3("E2_PREFIXO")[1])+PadR(cNota,TamSx3("E2_NUM")[1])
	If cParcela == E2_PARCELA .And. SE2->E2_ISS > 0
		dRet := SE2->E2_VENCREA
	EndIf
