		...
		While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			If dbSeek(xFilial("SX5")+"60"+RetFldProd(SB1->B1_COD,"B1_CODISS"))
				If !lNFeDesc  .And. nCont == 1
					If  cCodmun == "3300704-3156700"
						cNatOper := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(FWGetsx5("60", RetFldProd(SB1->B1_COD,"B1_CODISS")),1,55))),AllTrim(SubStr(FWGetsx5("60", RetFldProd(SB1->B1_COD,"B1_CODISS")),1,55))) + cNatOper
	    			Else
						cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(FWGetsx5("60", RetFldProd(SB1->B1_COD,"B1_CODISS")),1,55))),AllTrim(SubStr(FWGetsx5("60", RetFldProd(SB1->B1_COD,"B1_CODISS")),1,55)))    			
	    			EndIf
	    	    ElseIf nCont == 1 
	    	        If cCodmun == "3300704-3156700"
						cDescrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(FWGetsx5("60", RetFldProd(SB1->B1_COD,"B1_CODISS")),1,55))),AllTrim(SubStr(FWGetsx5("60", RetFldProd(SB1->B1_COD,"B1_CODISS")),1,55))) + cNatOper
	    			Else
						cDescrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(FWGetsx5("60", RetFldProd(SB1->B1_COD,"B1_CODISS")),1,55))),AllTrim(SubStr(FWGetsx5("60", RetFldProd(SB1->B1_COD,"B1_CODISS")),1,55)))    			
	    			EndIf
	    	    EndIf 
    		EndIf
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
cString += '<InscricaoMunicipalPrestador>'+AllTrim(SM0->M0_INSCM)+'</InscricaoMunicipalPrestador>'
