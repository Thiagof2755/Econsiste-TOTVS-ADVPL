...
ElseIf nTipo == 1
	...
	If lPergunte
		RPTStatus( {|lEnd| DANFEProc(@oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja, nTipo)}, "Imprimindo DANFE..." )
	EndIf
EndIf
...
If lImpSimpN
	...
	aSimpNac := {"",""}
	    ...
	    elseif Type("oDet["+Alltrim(Str(nS))+"]:_IMPOSTO:_ICMS:_ICMSSN900:_VCREDICMSSN:TEXT") <> "U"
			nS:= 0
	    	For nS := 1 To nDetImp
	    		If ztipo("oDet["+Alltrim(Str(nS))+"]:_IMPOSTO:_ICMS:_ICMSSN900:_VBC:TEXT") <> "U"
	 				nSnBaseIcm += Val(oDet[nS]:_IMPOSTO:_ICMS:_ICMSSN900:_VBC:TEXT)
				EndIf
				If ztipo("oDet["+Alltrim(Str(nS))+"]:_IMPOSTO:_ICMS:_ICMSSN900:_VCREDICMSSN:TEXT") <> "U"
...
If Type("oTransp:_Vol")<>"U"
	If ValType(oTransp:_Vol) == "A"
		...
		For nX := 1 to nLenVol
			nXAux := nX
			nVolume += IIF(!ztipo("oTransp:_Vol[nXAux]:_QVOL:TEXT")=="U",Val(oTransp:_Vol[nXAux]:_QVOL:TEXT),0)
		Next nX
...
For nZ := 1 To nLenDet
    ...
    lPontilhado	:= .F.
	If ztipo("oImposto:_Imposto")<>"U"
		If ztipo("oImposto:_Imposto:_ICMS")<>"U"
			nLenSit := Len(aSitTrib)
			For nY := 1 To nLenSit
				nPrivate2 := nY
				If ztipo("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2])<>"U" .Or. ztipo("oImposto:_Imposto:_ICMS:_ICMSST")<>"U"
					If ztipo("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_VBC:TEXT")<>"U"
					...
					ElseIf ztipo("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_MOTDESICMS") <> "U" .And. ztipo("oImposto:_PROD:_VDESC:TEXT") <> "U"   //SINIEF 25/12, efeitos a partir de 20.12.12
						...
						EndIf
					EndIf
					If ztipo("oImposto:_Imposto:_ICMS:_ICMSST")<>"U" // Tratamento para 4.0
			...
			If ztipo("oEmitente:_CRT") <> "U" .And. oEmitente:_CRT:TEXT == "1"
				nLenSit := Len(aSitSN)
				For nY := 1 To nLenSit
					nPrivate2 := nY
					If ztipo("oImposto:_Imposto:_ICMS:_ICMSSN"+aSitSN[nPrivate2])<>"U"
						If ztipo("oImposto:_Imposto:_ICMS:_ICMSSN"+aSitSN[nPrivate2]+":_VBC:TEXT")<>"U"
		...
		EndIf
		If ztipo("oImposto:_Imposto:_IPI")<>"U"
			If ztipo("oImposto:_Imposto:_IPI:_IPITrib:_vIPI:TEXT")<>"U"
				nValIPI := Val(oImposto:_Imposto:_IPI:_IPITrib:_vIPI:TEXT)
			EndIf
			If ztipo("oImposto:_Imposto:_IPI:_IPITrib:_pIPI:TEXT")<>"U"
	...
	aadd(aItens,{;
		...
		IIF(ztipo("oDet[nPrivate]:_Prod:_NCM")=="U","",oDet[nX]:_Prod:_NCM:TEXT),;
		cSitTrib,;
	...
	If (ztipo("oNf:_infnfe:_det[nPrivate]:_Infadprod:TEXT") <> "U" .Or. ztipo("oNf:_infnfe:_det:_Infadprod:TEXT") <> "U") .And. ( lImpAnfav .Or. lImpInfAd )
...
If Type("oNF:_INFNFE:_IDE:_NFREF")<>"U"
	...
	For Nx := 1 to Len(aInfNf)
		If ztipo("aInfNf["+Str(nX)+"]:_REFNFE:TEXT")<>"U" .And. !AllTrim(aInfNf[nx]:_REFNFE:TEXT)$cAux1
		...
		ElseIf ztipo("aInfNf["+Str(nX)+"]:_REFNF:_NNF:TEXT")<>"U" .And. !AllTrim(aInfNf[nx]:_REFNF:_NNF:TEXT)$cAux2 .And. lNFori2
   		...
   		For nX := 1 To nLenDet
	   		...
	   		If ztipo("oImposto:_Imposto")<>"U"
		 		If ztipo("oImposto:_Imposto:_ICMS")<>"U"
		 			...
		 			For nY := 1 To nLenSit
		 				nPrivate2 := nY
		 				If ztipo("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2])<>"U"
		 					If ztipo("oImposto:_IMPOSTO:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_VBCST:TEXT")<>"U"
...
// oDanfe:Say(052,098, "Identificação do emitente",oFont12N:oFont) // Alteracao 06/06/2019 Jonathan
nLinCalc	:=	065
...
For nX := 1 To nForTo
	// oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*25)+1),25), oFont12N:oFont ) // Alteracao 06/06/2019 Jonathan
	nLinCalc+=10
Next nX
...
nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
For nX := 1 To nForTo
	// oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*40)+1),40),oFont08N:oFont) // Alteracao 06/06/2019 Jonathan
	nLinCalc+=10
Next nX
...
If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
	...
	For nX := 1 To nForTo
		...
		nLinCalc+=10
	Next nX
	// oDanfe:Say(nLinCalc,098, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont) // Alteracao 06/06/2019 Jonathan
	nLinCalc+=10
	// oDanfe:Say(nLinCalc,098, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont) // Alteracao 06/06/2019 Jonathan
Else
	// oDanfe:Say(nLinCalc,098, NoChar(oEmitente:_EnderEmit:_xBairro:Text,lConverte)+" Cep:"+TransForm(IIF(Type("oEmitente:_EnderEmit:_Cep")=="U","",oEmitente:_EnderEmit:_Cep:Text),"@r 99999-999"),oFont08N:oFont) // Alteracao 06/06/2019 Jonathan
	nLinCalc+=10
	// oDanfe:Say(nLinCalc,098, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont) // Alteracao 06/06/2019 Jonathan
	nLinCalc+=10
	// oDanfe:Say(nLinCalc,098, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont) // Alteracao 06/06/2019 Jonathan
...
If nfolha==1
	...
	Else
		...
		// Alteracao 06/06/2019 Jonathan (Logo conforme empresa com tamanho personalizado)
		cLogo := GetSrvProfString("Startpath","") + "Lg" + cEmpAnt + cFilAnt + ".JPG"
		oDanfe:SayBitmap(043,003,cLogo,245,096) // oDanfe:SayBitmap(045,001,cLogo,080,090)
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > nMaxItemP2
			...
			// oDanfe:Say(010+nLinhavers,098, "Identificação do emitente",oFont12N:oFont) // Alteracao 06/06/2019 Jonathan
			...
			If ztipo("oEmitente:_EnderEmit:_xCpl") <> "U"
				cStrAux		:=	"Complemento: "+AllTrim(NoChar(oEmitente:_EnderEmit:_xCpl:TEXT,lConverte))
				nForTo		:=	Len(cStrAux)/40
				nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
				For nX := 1 To nForTo
				...
				If ztipo("oEmitente:_EnderEmit:_Cep")<>"U"
					cStrAux		+=	" Cep:"+TransForm(oEmitente:_EnderEmit:_Cep:Text,"@r 99999-999")
				EndIf
				nForTo		:=	Len(cStrAux)/40
				nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
			...
			Else
				...
				// Alteracao 06/06/2019 Jonathan (Logo conforme empresa com tamanho personalizado)
				cLogo := GetSrvProfString("Startpath","") + "Lg" + cEmpAnt + cFilAnt + ".JPG"
				oDanfe:SayBitmap(043,003,cLogo,245,096) // oDanfe:SayBitmap(002+nLinhavers,003,cLogo,085,085)
			...
			oDanfe:Say(143+nLinhavers,002,IIf(ztipo("oEmitente:_IE:TEXT")<>"U",oEmitente:_IE:TEXT,""),oFont08:oFont)
			oDanfe:Say(135+nLinhavers,205,"INSC.ESTADUAL DO SUBST.TRIB.",oFont08N:oFont)
			oDanfe:Say(143+nLinhavers,205,IIf(ztipo("oEmitente:_IEST:TEXT")<>"U",oEmitente:_IEST:TEXT,""),oFont08:oFont)
			oDanfe:Say(135+nLinhavers,405,"CNPJ/CPF",oFont08N:oFont)
			Do Case
				Case ztipo("oEmitente:_CNPJ")=="O"
					cAux := TransForm(oEmitente:_CNPJ:TEXT,"@r 99.999.999/9999-99")
				Case ztipo("oEmitente:_CPF")=="O"
					cAux := TransForm(oEmitente:_CPF:TEXT,"@r 999.999.999-99")
				OtherWise
					cAux := Space(14)
			EndCase
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		// oDanfe:Say(010,098, "Identificação do emitente",oFont12N:oFont) // Alteracao 06/06/2019 Jonathan
		nLinCalc	:=	023
		cStrAux		:=	AllTrim(NoChar(oEmitente:_xNome:Text,lConverte))
		nForTo		:=	Len(cStrAux)/25
		nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
		...
		Else
			...
			// Alteracao 06/06/2019 Jonathan (Logo conforme empresa com tamanho personalizado)
			cLogo := GetSrvProfString("Startpath","") + "Lg" + cEmpAnt + cFilAnt + ".JPG"
			oDanfe:SayBitmap(043,003,cLogo,245,096) // oDanfe:SayBitmap(000,000,cLogo,095,096)
		...
		oDanfe:Say(143,002,IIf(ztipo("oEmitente:_IE:TEXT")<>"U",oEmitente:_IE:TEXT,""),oFont08:oFont)
		oDanfe:Say(135,205,"INSC.ESTADUAL DO SUBST.TRIB.",oFont08N:oFont)
		oDanfe:Say(143,205,IIf(ztipo("oEmitente:_IEST:TEXT")<>"U",oEmitente:_IEST:TEXT,""),oFont08:oFont)
...
If lMensagens
	...
	If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
		...
		// oDanfe:Say(nLinCalc,098, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont) // Alteracao 06/06/2019 Jonathan
	Else
		// oDanfe:Say(nLinCalc,098, oEmitente:_EnderEmit:_xBairro:Text+" Cep:"+TransForm(IIF(Type("oEmitente:_EnderEmit:_Cep")=="U","",oEmitente:_EnderEmit:_Cep:Text),"@r 99999-999"),oFont08N:oFont) // Alteracao 06/06/2019 Jonathan
		nLinCalc+=10
		// oDanfe:Say(nLinCalc,098, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont) // Alteracao 06/06/2019 Jonathan
		nLinCalc+=10
		// oDanfe:Say(nLinCalc,098, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont) // Alteracao 06/06/2019 Jonathan
	...
	If oWS:RETORNANOTASNX()
		If Len(oWs:oWSRETORNANOTASNXRESULT:OWSNOTAS:OWSNFES5) > 0
			For nX := 1 To Len(oWs:oWSRETORNANOTASNXRESULT:OWSNOTAS:OWSNFES5)
				...
				cModTrans		  := IIf(ztipo("oNFeRet:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT") <> "U",IIf (!Empty("oNFeRet:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT"),oNFeRet:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT,1),1)
				...
				If !Empty(cProtocolo)
					...
					cDtHrRec		:= IIf(ztipo("oDHRecbto:_ProtNFE:_INFPROT:_DHRECBTO:TEXT")<>"U",oDHRecbto:_ProtNFE:_INFPROT:_DHRECBTO:TEXT,"")
